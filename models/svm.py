...
class SVM():
    def __init__(self, n_feats_to_select, kernel='rbf', C=1.0, gamma='scale', 
                 class_weight='balanced', get_summary=True, 
                 confusion_matrix=True, classification_report=True,
                 aus_score=True, plot_roc=True):
        """
        ...
        Args:
            ...
            class_weight: Class weight balancing ('balanced' or None)
        ...
        # Use linear SVM for RFE feature selection (required for coef_ attribute)
        # RFE needs feature importances which only linear kernel provides
        self.estimator = SVC(
            kernel='linear',
            C=C,
            class_weight=class_weight,
    ...
    def train(self, train_x: pd.DataFrame, train_y: pd.Series):
        ...
        # Scale features
        self.os_train_x_scaled = self.scaler.fit_transform(self.os_train_x)
        
        # Train model
        self.model.fit(self.os_train_x_scaled, train_y)
    ...
    def test(self, test_x: pd.DataFrame, test_y: pd.Series):
        """Test the SVM model and compute metrics."""
        os_test_x = test_x[self.selected_feats]
        os_test_x_scaled = self.scaler.transform(os_test_x)
        
        # Get predictions
        pred_y = self.model.predict(os_test_x_scaled)
        pred_y_prob = self.model.predict_proba(os_test_x_scaled)
        
        # Compute accuracy
        accuracy = accuracy_score(test_y, pred_y)
        print(f'\nAccuracy of SVM classifier on test set: {accuracy:.4f}')
    ...
    def _plot_roc(self, test_y, pred_y_prob, num_classes):
        ...
        if num_classes == 2:
            ...
            roc_auc = auc(fpr, tpr)
            plt.plot(fpr, tpr, label=f'SVM (AUC = {roc_auc:.4f})', linewidth=2)
        else:
        ...
        plt.grid(alpha=0.3)
        plt.savefig('SVM_ROC.png', dpi=300, bbox_inches='tight')
        print('\nROC curve saved as SVM_ROC.png')
        plt.close()
