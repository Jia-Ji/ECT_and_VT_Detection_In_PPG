...
from .vgg import vgg16_1D, vgg19_1D
...
class CompeleteModel(nn.Module):
    ...
    def __initialize_modules(self, config: DictConfig):
        ...
        # self.feat_extracter = vgg16_1D(**config.hyperparameters.feat_extracter)
        # self.feat_extracter = vgg19_1D(**config.hyperparameters.feat_extracter)
...
class EctopicsClassifier(pl.LightningModule):
    def __init__(self, 
                 ...
                 feature_extractor_lr_scale: float = 1.0,
                 **kwargs):
        super().__init__()
        
        # Store finetuning parameters
        self.freeze_feature_extractor = freeze_feature_extractor
        self.reinit_classifier = reinit_classifier
        self.feature_extractor_lr_scale = feature_extractor_lr_scale

        # save training config to logs
        if training_config is not None:
            if OmegaConf.is_config(training_config):
                # It's an OmegaConf object, convert to dict
                config_dict = OmegaConf.to_container(training_config, resolve=True)
            elif isinstance(training_config, dict):
                # Already a dict (e.g., when loading from checkpoint)
                config_dict = training_config
            elif isinstance(training_config, str):
                # If it's a string path (for backward compatibility)
        ...
        # Apply finetuning settings
        if self.freeze_feature_extractor:
            self._freeze_feature_extractor()
        
        if self.reinit_classifier:
            self._reinitialize_classifier()
        ...
        self.test_z_att = []
        # Store filenames for test samples
        self.test_filenames = []
    ...
    def _freeze_feature_extractor(self):
        ...
        print("=" * 60, flush=True)
        print("FINETUNING MODE: Freezing feature extractor", flush=True)
        print("=" * 60, flush=True)
        
        frozen_params = 0
        for name, param in self.model.feat_extracter.named_parameters():
        ...
        print(f"  Frozen parameters (feature extractor): {frozen_params:,}", flush=True)
        print(f"  Trainable parameters (classifier): {trainable_params:,}", flush=True)
        print("=" * 60, flush=True)
    
    def _reinitialize_classifier(self):
        """Reinitialize classifier weights (useful when finetuning on different task)."""
        print("Reinitializing classifier weights...", flush=True)
        for module in self.model.classifier.modules():
            if isinstance(module, nn.Linear):
                nn.init.kaiming_normal_(module.weight, mode='fan_out', nonlinearity='relu')
                if module.bias is not None:
                    nn.init.zeros_(module.bias)
        print("Classifier weights reinitialized.", flush=True)
    
    def unfreeze_feature_extractor(self):
        """Unfreeze all parameters in the feature extractor (for gradual unfreezing)."""
        print("Unfreezing feature extractor...", flush=True)
        for param in self.model.feat_extracter.parameters():
            param.requires_grad = True
        self.freeze_feature_extractor = False
        print("Feature extractor unfrozen.", flush=True)

    def configure_optimizers(self):
        
        if self.freeze_feature_extractor:
            # Only optimize classifier parameters when feature extractor is frozen
            params_to_optimize = [p for p in self.model.classifier.parameters() if p.requires_grad]
            print(f"Optimizer: Only optimizing classifier ({len(params_to_optimize)} parameter groups)", flush=True)
            optimizer = torch.optim.Adam(params_to_optimize, lr=self.lr, weight_decay=self.weight_decay)
        elif self.feature_extractor_lr_scale != 1.0:
            # Use different learning rates for feature extractor and classifier
            feat_extractor_params = list(self.model.feat_extracter.parameters())
            classifier_params = list(self.model.classifier.parameters())
            
            param_groups = [
                {'params': feat_extractor_params, 'lr': self.lr * self.feature_extractor_lr_scale},
                {'params': classifier_params, 'lr': self.lr}
            ]
            print(f"Optimizer: Feature extractor lr={self.lr * self.feature_extractor_lr_scale:.6f}, Classifier lr={self.lr:.6f}", flush=True)
            optimizer = torch.optim.Adam(param_groups, weight_decay=self.weight_decay)
        else:
            # Standard: optimize all parameters with the same learning rate
    ...
    def _compute_sqi_stratified_metrics(self):
        ...
        # Get filenames (truncate to match min_len)
        all_filenames = self.test_filenames[:min_len] if self.test_filenames else [''] * min_len
    ...
    def test_step(self, batch):
        ...
        # Store filenames for this batch
        if filenames is not None:
            if isinstance(filenames, (list, tuple)):
                self.test_filenames.extend([str(f) for f in filenames])
            elif isinstance(filenames, np.ndarray):
                self.test_filenames.extend([str(f) for f in filenames])
            else:
                # Single filename or unknown type
                self.test_filenames.append(str(filenames))
        else:
            # No filenames available, append empty strings for this batch
            batch_size = preds.shape[0]
            self.test_filenames.extend([''] * batch_size)
        
        # Store embeddings before and after attention
        self.test_z_backbone.append(self.model.feat_extracter.z_backbone.detach().cpu())
        # Handle case where z_att is None (e.g., VGG models without attention)
        if self.model.feat_extracter.z_att is not None:
            self.test_z_att.append(self.model.feat_extracter.z_att.detach().cpu())
        else:
            self.test_z_att.append(None)
